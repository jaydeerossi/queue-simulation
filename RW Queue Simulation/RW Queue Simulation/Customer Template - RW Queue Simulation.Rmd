...
author: Your name, Cogito TS
---


#Executive Summary
You have recently asked your Cogito TS to help optimize your Reporting Workbench performance through queue management. This document will provide quantitative evidence for making change(s) to your current queue structure. This methodolgy was developed by a Cogito TS and validated on multiple Epic organizations across the Epic community.

<Make your recommendation>

Any decisions made from this document should be preceded by a discussion with your Cogito TS. They can help you understand the opportunities and risks involved with queue management.
...
#Methodology
This analysis uses discrete event simulation to represent a single reporting workbench queue electronically. It can make claims about how RW *would have* performed had you taken the recommended changes prior to this analysis. It draws on two weeks of your historical RW run data. The breakdown of runs by queue can be found just below the following code snippet.

In order to model your queues effectively, the model incorporates several important settings on your queue definition/template. This includes maximum daemons, minimum daemons and others. Refer to the Generic Queueing Menu in Text or the code snippet below for definitions and values for these parameters.
...
#fill in these parameters according to the info in:
#d ^EQMENU > Queue Definitions > Edit Queue Definition > <Descriptor Name>
...
This analysis was validated by running a simulation with your current configuration. This simulation uses the HRN start and run times as input. In the below visualization, you can see how the simulated wait times compare to the observed wait times (in HRN).
